...
if ! command -v node >/dev/null 2>&1; then
    warn "Node.js non trovato — Lighthouse check saltato"
    exit 0
fi
...
if [[ ! -x "$LH_BIN" ]]; then
    if ! command -v npx >/dev/null 2>&1; then
        warn "lighthouse non trovato e npx non disponibile — Lighthouse check saltato"
        exit 0
    fi
...
for path in "${PATHS[@]}"; do
    path="/${path#/}"
    URL="${BASE_URL}${path}"
    REPORT="/tmp/lh-report-$$.json"

    echo -e "  Checking ${BOLD}${URL}${RESET} ..."
...
ok "Lighthouse check completato — tutti i budget rispettati"
exit 0
