...
- **`accessibility`**: riguarda i siti privati nello scope dell'European Accessibility Act (Direttiva UE 2019/882, dal 28 giugno 2025): tipicamente un sito che vende online (e-commerce, il caso più comune per un sito generico) o eroga uno dei servizi elencati dalla direttiva (bancari, trasporti, e-book, comunicazioni elettroniche…) — a meno che l'impresa sia una **microimpresa** (meno di 10 dipendenti e fatturato o bilancio annuo ≤2M€), sempre esente per i servizi (art. 4, par. 5). Il regime esatto per i privati in Italia (D.Lgs. 82/2022) va verificato caso per caso con un consulente legale: non è la stessa cosa della Legge 4/2004, che resta per la Pubblica Amministrazione. Lo stato di conformità lo scrive l'Engine (chiavi `acc*` di `basic.*.json`) in base a `nonAccessibili`, i contenuti non accessibili noti dichiarati nello slot: `{ descrizioneKey, motivo, alternativaKey? }`, testi in `addon.*.json`, `motivo` fra `'non-conformita'`, `'onere-sproporzionato'` e `'fuori-ambito'`. Senza voci il testo dice che il sito è progettato per rispettare WCAG 2.1 AA; con voci aggiunge che ha le criticità elencate e le elenca. `intro/` tiene titolo e una frase, `outro/` redazione e segnalazioni.
