...
| Deploy | `scripts/test/run-all.sh` | `./deploy.sh` (eseguito in fase di Pre-flight isolato, prima di rimpiazzare la produzione) |
...
# Nel deploy di produzione (fase isolata pre-flight)
bash deploy.sh
# (Se hai un'emergenza e devi saltare i test: bash deploy.sh --skip-tests)
```
