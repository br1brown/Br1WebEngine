...
### I design system diventano codice: factory function, estensione, palette con nome proprio (breaking)

Fin qui un design system era un dato statico (`Record<string, DesignSystemPreset>`) e i quattro override colore vivevano ancora in `global-settings.json`, doppiando la fonte di verità rispetto al preset. Tre cambi collegati:

- **Breaking**: ogni voce di `DESIGN_SYSTEM_PRESETS` è ora un `DesignSystemFactory` (`() => DesignSystemPreset`), non un oggetto letterale — stesso idioma di `pages: () => [...]` in `site.ts`. `resolveDesignSystemPreset` esegue la factory invece di leggere un valore. Chi legge `DESIGN_SYSTEM_PRESETS` direttamente (non tramite `shell.designSystem`) deve chiamarla.
- **Breaking**: `colorSecondary`/`colorBackground`/`colorText`/`colorInfo` rimossi da `global-settings.json`/schema/types — l'unico colore di identità che il JSON dichiara resta `colorTema`. I quattro override sono ora sempre e solo una proposta del design system attivo (`DesignSystemPreset.colorSecondary`/...), stessa matematica di `ThemeService.computePalette()`. Migrazione: un progetto che li usava nel JSON li sposta nel preset attivo (o in un'estensione di dominio).
- Nuovo `shell.designSystem` accetta anche un `DesignSystemFactory` importato direttamente, non solo il nome di un preset dell'Engine — la via per un design system di dominio, che non passa da nessun registro per nome.
- Nuovo `extendDesignSystem(base, patch)`: costruisce un design system per estensione di un altro (deep-merge di `roleChrome`/`customPalette`, override shallow per il resto) — la "classe padre da sovrascrivere" richiesta, fatta per composizione invece che con `class`/`extends` (nessun altro punto del template usa ereditarietà OOP per la configurazione).
- Nuovo `DesignSystemPreset.customPalette` (`Record<string, string>`): colori con nome proprio oltre ai quattro slot fissi — ogni voce passa nella stessa pipeline WCAG di `colorSecondary` (fill conforme al contrasto target + testo leggibile sopra, estratta in `ThemeService.computeAccentPair`, riusata anche per `colorSecondary` stesso) ed esce come coppia di CSS custom properties tone-adaptive `--color<Label>`/`--color<Label>Text`. Aggiunge, non sostituisce: i quattro slot semantici restano i default calcolati dal brand quando non esplicitamente scostati.
- Nuovo `components/shared/design-systems/esempio.design-system.ts` (Dominio): dimostra il pattern completo, `extendDesignSystem(DESIGN_SYSTEM_PRESETS.muro, { customPalette: {...} })`.
- Verificato dal vivo (Playwright): il design system di esempio, attivato temporaneamente, eredita correttamente `forceThemeTone: 'dark'`/`navSurface: 'body'`/`roleChrome` da `muro` (tema forzato, navbar/footer sullo sfondo pagina, pannello assente su `default` e presente su `legal`) e aggiunge due colori (`--colorBordeaux`/`--colorOro` + varianti `Text`) con contrasto WCAG calcolato correttamente in dark mode. Rimossa l'attivazione di prova, confermato che il sito demo senza `designSystem` (colori derivati dal solo `colorTema`, nessun campo JSON rimosso mai impostato) resta identico byte-per-byte a prima. `tsc`/build/lint/i18n/circular-deps/site-builder-check puliti.
...
- **Breaking**: `LeafPageInput.layout.showNav`/`showFooter`/`showPanel` rimossi. Migrazione: chi li usava per un bisogno che il ruolo/design system attivo già copre non deve fare nulla (il ruolo lo sostituisce); chi li usava per un bisogno vero non ancora mappato da nessun ruolo deve farlo mappare dal design system attivo (estendendo `roleChrome`) o cambiarne uno.
- Migrate le uniche due pagine del template che usavano l'escape hatch (`social-feed`/`social-feed/:slug` in `app.pages.ts`, `layout: { showPanel: false }`): rimosso, tornano pagine `'default'` a tutti gli effetti — con l'unico design system disponibile oggi che non differenzia i ruoli (`adaptive-light-panel`), il pannello torna visibile anche lì. Un domani un design system dedicato può ridargli un trattamento diverso via `roleChrome`, questa volta come proprietà del design system e non della singola pagina.
- Effetto collaterale coerente: anche `fitViewport` (vista full-bleed) perde la possibilità di riattivare il footer via `showFooter: true` — restava l'unico varco per riaprire lo stesso flag appena chiuso. In full-bleed il footer resta sempre nascosto, incondizionatamente (stesso spazio conteso di pannello/smoke, già esclusi strutturalmente), a prescindere da `role`/design system.
- Verificato dal vivo (Playwright): `designSystem: 'muro'` + una pagina `role: 'naked'` senza più nulla da poter scrivere in `layout` per riaccendere nav/footer/pannello — nessuno dei tre compare, coerente con prima ma stavolta senza che ci sia più un flag da provare a scavalcare. Sito senza `designSystem` → comportamento identico a prima su Home/pagine legali; `social-feed` mostra ora il pannello (trade-off accettato). `tsc`/build/lint/i18n/circular-deps/site-builder-check puliti.

### Nuovo `layout.role`: le pagine dichiarano CHE COSA sono, il design system decide COME appaiono

Fino a ieri ogni pagina che voleva un layout diverso dal default doveva disattivare a mano `showNav`/`showFooter`/`showPanel` — una decisione strutturale sparsa in ogni file di area, invece che vivere in un unico posto (il design system attivo). `layout.role` chiude il cerchio aperto da `designSystem` (voci sotto): un contratto fisso di 3 nomi (`'default'`, `'legal'`, `'naked'`) che ogni pagina dichiara, e che il design system attivo interpreta in nav/footer/pannello tramite un nuovo `DesignSystemPreset.roleChrome`.

- Nuovo `PageRole` (`design-system-presets.ts`): `'default'` | `'legal'` | `'naked'`. `'naked'` è l'unico ruolo **forzato**: niente navbar, niente footer, niente pannello — non sovrascrivibile né dal design system né dal `layout.showNav`/`showFooter`/`showPanel` della stessa pagina (l'unica eccezione all'ordine di priorità sotto).
- Nuovo `DesignSystemPreset.roleChrome` (`{ default?, legal? }`, ciascuno `{ showNav?, showFooter?, showPanel? }`): come il design system attivo rende i ruoli `'default'`/`'legal'`. Campo assente o ruolo non mappato → nessuna differenza dal comportamento pre-ruoli (zero cambiamenti per i 7 preset preesistenti, nessuno dei quali lo popola).
- Nuovo preset `muro` (il caso che ha motivato questo lavoro — Agnese Subacchi): palette fissa scura, **nessun pannello** sulle pagine di contenuto (`roleChrome.default: { showPanel: false }`) — il contenuto vive direttamente sullo sfondo, senza la card chiara che spezzerebbe l'uniformità. Le pagine legali restano un'eccezione voluta: `roleChrome.legal: { showPanel: true }`, il pannello torna per la leggibilità del testo lungo.
- Nuovo `layout.role` su `LeafPageInput` (`siteBuilder.ts`). Ordine di priorità con cui `normalizeSitePage` risolve nav/footer/pannello: `'naked'` (fisso) > override esplicito della pagina (`layout.showNav`/`showFooter`/`showPanel`) > default del ruolo dato dal design system attivo (`roleChrome`) > default globale di sito (`shell.showNav`/`showFooter`/`showPanel`, invariato). `buildFinalConfig` ora espone anche il preset risolto, così `normalizeSitePages` lo riusa senza un secondo lookup.
- Migrate le pagine legali generate dall'Engine (`legal-pages.ts`, `buildPolicySection`) a `role: 'legal'` — prima di questo lavoro non avevano un ruolo dichiarato.
- Verificato dal vivo (Playwright, `designSystem: 'muro'`): pagina di contenuto default → pannello assente, navbar/footer presenti; pagina con `role: 'naked'` e override espliciti `showNav`/`showFooter`/`showPanel: true` nello stesso `layout` → tutti e tre forzatamente assenti (la forzatura vince sull'override esplicito); pagina legale → pannello presente. Rimosso il `designSystem`/`role` di prova, rigenerato `generate:statics`, confermato lo stesso comportamento di prima (nav/footer/pannello sempre presenti) sul sito senza design system. `tsc`/build/lint/i18n/circular-deps/site-builder-check puliti.
...
- Nuovo `site.forceThemeTone: 'light' | 'dark'` (`global-settings.json`, come `colorTema` — assente segue l'OS come sempre). Copre tutte e tre le fasi: SSR (`app.config.server.ts`), script anti-flash pre-idratazione (`theme-init.js`, generato da `generate-statics.ts` senza `matchMedia` quando forzato) e `ThemeService` runtime (nessun listener `prefers-color-scheme` montato quando forzato — un cambio OS non ha più alcun effetto).
- **Breaking**: `shell.panelForcedLight: boolean` (`site.ts`) diventa `shell.panelSurface: 'light' | 'dark' | 'auto'` — stesso ruolo (tono del pannello contenuti indipendente dall'OS), ma a tre stati: `'auto'` (nuovo) segue l'ambiente, `'dark'` (nuovo, la classe CSS `.panel-dark` esisteva già ma non era mai applicata da nessun binding — codice morto, ora cablato) forza scuro, `'light'` è il default e corrisponde al vecchio `true`. Migrazione: `panelForcedLight: true` → `panelSurface: 'light'` (o rimuovilo, è il default); `panelForcedLight: false` → `panelSurface: 'auto'`.
- `ThemeService.panelForcedLight`/`panelBootstrapTheme` (due campi scollegati, letti da due binding indipendenti in `app.component.html` — motivo per cui un fix parziale al solo `data-bs-theme` lasciava il pannello ancora chiaro) diventano un solo `panelTone: 'light' | 'dark' | null`, unica fonte di verità sia per l'attributo Bootstrap sia per le classi CSS.
- **Mutua esclusione**: `forceThemeTone` e `panelSurface` non possono essere entrambi attivi — quando il primo è impostato, il secondo è ignorato (`panelTone` è sempre `null`, il pannello segue l'ambiente già fissato) con un `console.warn` in dev mode se `panelSurface` non è `'auto'`, invece di lasciarlo silenziosamente senza effetto.
- Verificato dal vivo: con `forceThemeTone: 'dark'` e l'OS emulato in `light`, `<html data-bs-theme>` resta `dark` e il pannello contenuti (prima sempre chiaro di default) ora ha lo sfondo scuro coerente con il resto della pagina — nessun riquadro chiaro a rompere il design. Warning di mutua esclusione confermato in console. `tsc --noEmit`/build/lint/i18n/circular-deps/site-builder-check puliti.
